...
namespace AuroraLoader
{
	public partial class FormMain : Form
	{
		...
		private AuroraVersion _installedVersion;
		...
		public FormMain(IConfiguration configuration, AuroraLoaderRegistry registry)
		{
			...
			_registry = registry;
			_installedVersion = AuroraVersionUtilities.GetCurrentlyInstalledAuroraVersion(_configuration, _registry);
		}
		...
		private void RefreshAuroraInstallData()
		{
			...
			LabelChecksum.Text = $"Aurora checksum: {_installedVersion.Checksum}";
			LabelVersion.Text = $"Aurora version: {_installedVersion.Version}";
			...
			if (_registry.AuroraVersions.Any() && _installedVersion.CompareTo(_registry.AuroraVersions.Max()) < 1)
			{
		...
		private void LoadMods()
		{
			...
			{
				if (mod.WorksForVersion(_installedVersion))
				{
